<html lang="ru">
<head>
    <title>Изменение информации конкретного пользователя</title>
</head>
<body>
<?php
session_start();
$link = mysqli_connect('localhost', 'root' , '','bd');
$sql = mysqli_query($link, "SELECT `id`, `name`, `surname`,`login`,`password`,`lang` FROM `users` WHERE `id`={$_GET['red_id']}");
$log = $_SESSION['login'];
...
if (isset($_POST["Name"])) {
    ...
    if (isset($_GET['red_id'])) {
        ...
        $sql = mysqli_query($link, "UPDATE `users` SET `name` = '{$_POST['Name']}',`surname` = '{$_POST['Surname']}',`login` = '{$_POST['Login']}',
                                                            `password` = '{$_POST['Password']}',`lang` = '{$_POST['Language']}' WHERE `id`={$_GET['red_id']}");
    } else {

        $sql = mysqli_query($link, "INSERT INTO `users` (`name`, `surname`,`login`,`password`,`lang`) VALUES ('{$_POST['Name']}', '{$_POST['Surname']}','{$_POST['Login']}',
